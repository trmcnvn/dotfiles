...
A user-global Pi extension that exposes blocking `delegate` and read-only `read_agent_activity` tools for scoped implementation, research, and independent review in temporary sibling Herdr panes in the parent's tab.
...
Herdr owns sibling-pane creation, Pi startup, lifecycle waits, and pane identity. The extension never starts tmux or a raw child Pi subprocess. It resolves the calling pane with `pane current --current`, inspects its layout, and splits that exact pane in the parent's tab with the caller cwd, child environment, and `--no-focus`. Wide panes split right (column count at least three times row count); narrower or taller panes split down. The UI-focused pane is not used as the launch target. Delegation creates no tabs or workspaces; the returned pane, tab, and workspace identities are validated and pinned. A narrowly bounded second `agent start` attempt is allowed only after the known `agent_pane_busy` shell-readiness rejection and a matching, agent-free pane lookup. Each role body is written with mode `0600` to the worker's private artifact directory and its absolute path is passed to `--append-system-prompt`; multiline role text is never placed in the launch argv.
...
Builder is retired. Only Worker, Scout, and Reviewer are callable roles; `role: "builder"` is rejected, not aliased or launched. Persisted builders keep their role, native session/name/pane, and original fingerprint unchanged for safe ownership cleanup or explicit replacement with Worker. Legacy follow-ups return `role_retired` before configuration loading, state publication, or task delivery. No `builder.md` is required. Edit `worker.md` for new defaults. Configuration drift rejects ordinary Worker follow-ups; only explicit replacement selects new configuration. No model identities are embedded in workflow prose.

The v1 task envelope, child result artifact/status protocol, and old persisted records remain compatible. Historical results (including `builder_followups`) are not rewritten; new runtime output uses only `worker_followups` for retention. Scout/Reviewer cleanup and parent `agent_settled` behavior share the existing ownership checks.
