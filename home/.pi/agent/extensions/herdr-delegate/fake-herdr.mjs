...
if (command === "pane layout") {
	response({ layout: { panes: [{ pane_id: "caller", rect: { width: 120, height: 40 } }] } });
	...
	response({ pane: { pane_id: "worker-pane" } });
	...
	state.session = `/tmp/${state.agentName}.jsonl`;
	...
	response({ pane: { pane_id: "worker-pane", agent: state.scenario === "startup-blocked" ? "pi" : null } });
