...
async function getVcsKind(pi: ExtensionAPI): Promise<VcsKind | null> {
	const jjRoot = await pi.exec("jj", ["root"]);
	if (jjRoot.code === 0) return "jj";
	const gitRoot = await pi.exec("git", ["rev-parse", "--git-dir"]);
	return gitRoot.code === 0 ? "git" : null;
}
...
const JJ_LOCAL_CHANGES_REVIEW_INSTRUCTIONS =
	"Also include local working-copy changes from this branch. Use `jj status`, `jj diff`, and `jj diff --stat` so local fixes are part of this review cycle.";
...
const JJ_BASE_BRANCH_PROMPT =
	"Review the code changes against the base bookmark/revision '{branch}'. Use `jj status`, `jj diff --stat --from {branch} --to @`, and `jj diff --from {branch} --to @` to inspect the changes relative to {branch}. Provide prioritized, actionable findings.";
...
const JJ_PULL_REQUEST_PROMPT =
	'Review pull request #{prNumber} ("{title}") against the base bookmark/revision \'{baseBranch}\'. Use `jj status`, `jj diff --stat --from {baseBranch} --to @`, and `jj diff --from {baseBranch} --to @` to inspect the changes that would be merged. Provide prioritized, actionable findings.';
...
async function getMergeBase(
	...
	if ((await getVcsKind(pi)) === "jj") return null;
...
async function getLocalBranches(pi: ExtensionAPI): Promise<string[]> {
	if ((await getVcsKind(pi)) === "jj") {
		const { stdout, code } = await pi.exec("jj", ["bookmark", "list"]);
		if (code !== 0) return [];
		return stdout
			.trim()
			.split("\n")
...
async function hasUncommittedChanges(pi: ExtensionAPI): Promise<boolean> {
	if ((await getVcsKind(pi)) === "jj") {
		const { stdout, code } = await pi.exec("jj", ["status"]);
		return code === 0 && stdout.includes("Working copy changes:");
	}
...
async function getCurrentBranch(pi: ExtensionAPI): Promise<string | null> {
	if ((await getVcsKind(pi)) === "jj") {
		const { stdout, code } = await pi.exec("jj", ["log", "-r", "@", "--no-graph", "-T", 'bookmarks.join(" ") ++ "\\n"']);
		const bookmark = stdout.trim().split(/\s+/).find((name) => name.length > 0);
		return code === 0 && bookmark ? bookmark : null;
	}
...
async function buildReviewPrompt(
	...
	const localChangesInstructions = vcsKind === "jj" ? JJ_LOCAL_CHANGES_REVIEW_INSTRUCTIONS : GIT_LOCAL_CHANGES_REVIEW_INSTRUCTIONS;

	switch (target.type) {
		case "uncommitted":
			return vcsKind === "jj"
				? "Review the current Jujutsu working-copy changes. Run `jj status`, `jj diff --stat`, and `jj diff` to inspect the changes. Provide prioritized findings."
				: UNCOMMITTED_PROMPT;

		case "baseBranch": {
			if (vcsKind === "jj") {
				const basePrompt = JJ_BASE_BRANCH_PROMPT.replace(/{branch}/g, target.branch);
				return includeLocalChanges ? `${basePrompt} ${localChangesInstructions}` : basePrompt;
			}
		...
		case "pullRequest": {
			if (vcsKind === "jj") {
				const basePrompt = JJ_PULL_REQUEST_PROMPT
...
export default function reviewExtension(pi: ExtensionAPI) {
	...
	pi.registerCommand("review", {
		...
		handler: async (args, ctx) => {
			...
			// Check if we're in a supported repository
			const vcsKind = await getVcsKind(pi);
			if (vcsKind === null) {
