...
const EXTRACTION_MODEL_IDS = ["gpt-5.6-luna-fast", "gpt-5.6-luna"];
...
export default function (pi: ExtensionAPI) {
	const answerHandler = async (ctx: ExtensionContext) => {
			...
			const extractionModel = EXTRACTION_MODEL_IDS
				.map((modelId) => ctx.modelRegistry.find(EXTRACTION_PROVIDER, modelId))
				.find((model) => model !== undefined);
			if (!extractionModel) {
				ctx.ui.notify(
					`Extraction models not available: ${EXTRACTION_MODEL_IDS.join(", ")}`,
					"error",
				);
				return;
			}
